#!/usr/bin/env node
const JRE_PATH = process.env.JAVA_HOME ? `${process.env.JAVA_HOME}/bin/` : ''
const RCTL_DATA = process.env.RCTL_DATA ? process.env.RCTL_DATA : require('os').homedir()
const CONFIG_PATH=`${RCTL_DATA}/.routr-access.json`
...
const cmd = `${JRE_PATH}java -Dlog4j.configurationFile=${__dirname}/log4j2.yml -Dfile.encoding=UTF-8 -cp ${__dirname}/libs/routr-ctl-all.jar -Dcom.sun.net.ssl.checkRevocation=true io.routr.ctl.Main ${args}`
